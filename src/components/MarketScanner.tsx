        ...
        if (item.decision === 'BUY' || item.decision === 'SELL') {
          ...
          if (item.analysis) {
            sendTelegramSignal({
              ...
              entryPrice: item.analysis.trade_setup?.entry_price,
              stopLoss: item.analysis.trade_setup?.stop_loss,
              breakevenTriggerPrice: item.analysis.trade_setup?.beTriggerPrice,
              target50PercentPrice: item.analysis.trade_setup?.target50PercentPrice,
              lockProfitPrice: item.analysis.trade_setup?.quadScaleOut?.tp1_0_5r || item.analysis.trade_setup?.beTriggerPrice,
            ...
            if (item.decision !== updatedAnalysis.decision && (updatedAnalysis.decision === 'BUY' || updatedAnalysis.decision === 'SELL')) {
              ...
              if (liveSop >= 4) {
                ...
                sendTelegramSignal({
                  ...
                  entryPrice: updatedAnalysis.trade_setup?.entry_price,
                  stopLoss: updatedAnalysis.trade_setup?.stop_loss,
                  breakevenTriggerPrice: updatedAnalysis.trade_setup?.beTriggerPrice,
                  target50PercentPrice: updatedAnalysis.trade_setup?.target50PercentPrice,
                  lockProfitPrice: updatedAnalysis.trade_setup?.quadScaleOut?.tp1_0_5r || updatedAnalysis.trade_setup?.beTriggerPrice,
